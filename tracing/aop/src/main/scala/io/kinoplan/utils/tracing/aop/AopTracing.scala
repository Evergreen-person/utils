...
import io.kinoplan.utils.tracing.core.{Tags, Trace}

object AopTracing {

  def traceWithMethodNamesAndArgs[F[_]: Sync](
    tags: Tags = Map.empty
  ): Aspect.Weave.Function[F, Show, *] ~> Trace[F, *] =
    new (Aspect.Weave.Function[F, Show, *] ~> Trace[F, *]) {

      override def apply[A](fa: Weave.Function[F, Show, A]): Trace[F, A] =
