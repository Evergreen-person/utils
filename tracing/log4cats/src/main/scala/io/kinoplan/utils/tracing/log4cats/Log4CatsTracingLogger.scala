...
import io.kinoplan.utils.tracing.core.{Context, Trace, TracingLogger}

object Log4CatsTracingLogger {

  implicit def tracingContextInstance[F[_]: Sync]: TracingLogger[Trace[F, *]] =
    new TracingLogger[Trace[F, *]] {
      ...
      override def info[A](value: => String)(implicit
        evidence: ClassTag[A]
      ): Trace[F, Unit] = Kleisli { tracingContext =>
